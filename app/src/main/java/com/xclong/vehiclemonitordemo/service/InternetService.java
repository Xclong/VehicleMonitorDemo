...
public class InternetService extends Service implements Runnable {
    ...
    private void connectService() {
        try {
            socket = new Socket();
            SocketAddress socAddress = new InetSocketAddress("", 8000);
            ...
            ex.printStackTrace();
            workStatus = Const.TAG_CONNECTFAILURE;// 如果是网络连接出错了，则提示网络连接错误
            return;
       ...
       /* if (!isNetworkConnected())// 如果当前网络连接不可用,直接提示网络连接不可用，并退出执行。
        {
            ...
            workStatus = Const.TAG_CONNECTFAILURE;
            return;
        }*/
    ...
    public void run() {
        try {
            ...
            workStatus = Const.TAG_CONNECTFAILURE;// 如果出现异常，提示网络连接出现问题。
            ex.printStackTrace();
        }
    }
